...
from ..models import AggregatorInput, FaultParty, Outcome, CaseResponse
...
def list_cases(merchant_id: Optional[str] = None):
    all_rows = wf.rows("cases")
    selected = [r for r in all_rows if not merchant_id or r["case_payload"]["merchant"]["id"] == merchant_id]
    selected.sort(key=lambda r: r["created_at"], reverse=True)
    # Canonical case JSON lets dashboards load workflows without N+1 queries.
    return [dict(case_id=r["id"], status=r["status"], case=r["case_payload"],
                 verdict=r["case_payload"].get("workflow", {}).get("verdict"),
                 check_results=r["case_payload"].get("workflow", {}).get("checks", [])) for r in selected]
...
def add_evidence(case_id: str, body: EvidenceBody):
    with wf.LOCK:
        ...
        case = build_case(order_id=old.order.id, trigger=old.trigger, complaint_type=old.complaint.type,
                          description=body.description or old.complaint.description, photo_url=body.photo_url or old.complaint.photo_url)
        case.case_id = old.case_id
        case.complaint.id = old.complaint.id
        case.workflow = old.workflow
        case.workflow.setdefault("additional_evidence", []).append(dict(photo_url=body.photo_url, description=body.description, at=wf.now()))
        wf.event(case, "Additional customer evidence received")
        wf.get_supabase().table("complaints").update(dict(photo_url=case.complaint.photo_url, description=case.complaint.description)).eq("id", case.complaint.id).execute()
...
def partner(case_id: str, body: PartnerBody):
    with wf.LOCK:
        ...
        case = response.case
        case.workflow["partner"] = dict(status="disputed" if body.action == "dispute" else "accepted", reason=body.reason, at=wf.now())
...
def support(case_id: str, body: SupportBody):
    with wf.LOCK:
        response = get_case(case_id)
        case, verdict = response.case, response.verdict
        if not verdict:
            raise HTTPException(409, "No assessment available")
        public_model = "NEITHER" if verdict.model_prediction == "EXTERNAL" else (verdict.model_prediction or verdict.fault_party.value.upper())
        human = body.action.upper() if body.action in ("merchant", "rider", "neither") else case.workflow.get("human_verdict", verdict.fault_party.value.upper())
        ...
        if body.action == "request_evidence":
            ...
            case.workflow.setdefault("feedback", []).append(dict(case_id=case_id, model_prediction=verdict.model_prediction,
                model_confidence=verdict.confidence, human_verdict=human, confirmed=human == public_model,
                override_reason=body.reason, action=body.action, resolved_at=wf.now()))
...
def notifications(customer_id: Optional[str] = None, order_id: Optional[str] = None):
    if order_id:
    ...
    return [n for row in wf.rows("cases") for n in row["case_payload"].get("workflow", {}).get("zone_incident", {}).get("notifications", []) if n["customer_id"] == customer_id]
...
def ops():
    ...
    return dict(counts=dict(active_orders=sum(o["status"] in wf.OPEN for o in orders), late_orders=sum(o["status"] in wf.OPEN and o["is_late_flagged"] for o in orders),
        open_incidents=sum(c["status"] != "resolved" for c in cases), zone_incidents=len(zones),
        auto_resolved=sum(bool(c["case"].get("workflow", {}).get("refund")) for c in cases), support_review=len(tickets)),
        zone_incidents=zones, cases=cases, support_case_ids=[t["case_id"] for t in tickets])
...
def admin():
    ...
    for customer in wf.rows("customers"):
        ...
        accounts.append(dict(id=cid, name=customer["name"], orders=sum(o["customer_id"] == cid for o in orders), claims=claims,
            refunds=sum(r["customer_id"] == cid for r in refunds), manual_review=any(r["case_payload"].get("workflow", {}).get("account_manual_review") for r in customer_cases)))
    ...
    return dict(accounts=accounts, reviewed_cases=len(reviewed), overrides=overrides,
                agreement_rate=(len(reviewed)-overrides)/len(reviewed) if reviewed else None, repeat_faults=repeat, feedback=feedback)
...
def flag(customer_id: str, body: FlagBody):
    with wf.LOCK:
